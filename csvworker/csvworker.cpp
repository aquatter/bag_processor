...
CsvWorker::CsvWorker(char delim, unsigned int rows_count,
...
void CsvWorker::loadFile(std::string filename,
                         std::deque<std::string> &readed_data) {
  // File to read
  FILE *fin;
...
row &CsvWorker::getRowRef(unsigned int row_n) {
  ...
  {
    cout << "Wrong row index: " << row_n
         << ", but max index is: " << this->rows_count - 1 << endl;
    throw;
  }
  return data[row_n];
}
...
std::string &CsvWorker::getFieldRef(unsigned int row_n, unsigned int col_n) {
  ...
  {
    cout << "Wrong field index: " << col_n
         << ", but max index is: " << this->cols_count - 1 << endl;
    throw;
  }
  return rw[col_n];
}
