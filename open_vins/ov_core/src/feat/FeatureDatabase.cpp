...
void FeatureDatabase::update_feature(size_t id, int64_t timestamp, size_t cam_id, float u, float v, float u_n, float v_n) {
...
std::vector<std::shared_ptr<Feature>> FeatureDatabase::features_not_containing_newer(int64_t timestamp, bool remove, bool skip_deleted) {
...
std::vector<std::shared_ptr<Feature>> FeatureDatabase::features_containing_older(int64_t timestamp, bool remove, bool skip_deleted) {
...
std::vector<std::shared_ptr<Feature>> FeatureDatabase::features_containing(int64_t timestamp, bool remove, bool skip_deleted) {
...
void FeatureDatabase::cleanup_measurements(int64_t timestamp) {
  std::lock_guard<std::mutex> lck(mtx);
  for (auto it = features_idlookup.begin(); it != features_idlookup.end();) {
...
void FeatureDatabase::cleanup_measurements_exact(int64_t timestamp) {
  std::lock_guard<std::mutex> lck(mtx);
  std::vector<int64_t> timestamps = {timestamp};
  for (auto it = features_idlookup.begin(); it != features_idlookup.end();) {
    // Remove the older measurements
