...
public:
  ...
  std::unordered_map<size_t, std::vector<Eigen::Vector2f>> uvs;

  /// UV normalized coordinates that this feature has been seen from (mapped by camera ID)
  std::unordered_map<size_t, std::vector<Eigen::Vector2f>> uvs_norm;

  /// Timestamps of each UV measurement (mapped by camera ID)
  std::unordered_map<size_t, std::vector<int64_t>> timestamps;
   ...
   */
  void clean_old_measurements(const std::vector<int64_t> &valid_times);
   ...
   */
  void clean_invalid_measurements(const std::vector<int64_t> &invalid_times);
   ...
   */
  void clean_older_measurements(int64_t timestamp);
};
