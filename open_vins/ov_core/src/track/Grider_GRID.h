...
public:
  ...
  static void perform_griding(const cv::Mat &img, const cv::Mat &mask, const std::vector<std::pair<int, int>> &valid_locs,
                              ...
                              bool nonmaxSuppression, std::span<const cv::Rect> boxes = {}) {
    ...
    parallel_for_(cv::Range(0, (int)valid_locs.size()), LambdaBody([&](const cv::Range &range) {
                    for (int r = range.start; r < range.end; r++) {
                      ...
                      std::sort(pts_new.begin(), pts_new.end(), [](const auto &a, const auto &b) { return a.response > b.response; });
    ...
    if (not boxes.empty()) {
      ...
      for (auto &&box : boxes) {
        const int delta_x{static_cast<int>(static_cast<float>(box.width) * 0.05f + 0.5f)};
        const int delta_y{static_cast<int>(static_cast<float>(box.height) * 0.05f + 0.5f)};
        const cv::Point m0{std::max(box.x - delta_x, 0), std::max(box.y - delta_y, 0)};
        const cv::Point m1{std::min(box.x + box.width + delta_x, img.cols - 1), std::min(box.y + box.height + delta_y, img.rows - 1)};

        std::vector<cv::KeyPoint> pts_new;
        cv::FAST(img(cv::Rect{m0, m1}), pts_new, threshold, nonmaxSuppression);
        std::sort(pts_new.begin(), pts_new.end(), [](const auto &a, const auto &b) { return a.response > b.response; });

        for (auto &&p : pts_new) {
          p.pt.x += static_cast<float>(m0.x);
          p.pt.y += static_cast<float>(m0.y);
          pts.push_back(p);
        }
      }
    }
