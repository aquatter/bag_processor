...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  if (pts_last[cam_id].empty()) {
    ...
    perform_detection_monocular(imgpyr, mask, good_left, good_ids_left, message.boxes);
  ...
  perform_detection_monocular(img_pyramid_last[cam_id], img_mask_last[cam_id], pts_left_old, ids_left_old, message.boxes);
...
void TrackKLT::perform_detection_monocular(const std::vector<cv::Mat> &img0pyr, const cv::Mat &mask0, std::vector<cv::KeyPoint> &pts0,
                                           std::vector<size_t> &ids0, std::span<const cv::Rect> boxes) {
  ...
  std::vector<cv::KeyPoint> pts0_ext;
  Grider_GRID::perform_griding(img0pyr.at(0), mask0_updated, valid_locs, pts0_ext, num_features, grid_x, grid_y, threshold, true, boxes);
